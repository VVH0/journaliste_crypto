def compter_les_nb(tabs) # définition d'une méthode
  ...
  tabs.each do |tab| # chaque élément du tableau tabs est passé en revue et devient tab
    ...
    count += 1 # si cette hypothèse est vérifiée, je rajoute 1 au compteur à chaque fois
    end
  end
  return count # je renvoie le comptage final de la méthode
end
...
def compter_start_maj(tabs)
  count=0 # mise en place d'un compteur
  tabs.each do |tab| # chaque élément du tableau tabs est passé en revue et devient tab
    if tab[1]=~/[A-Z]/ # je vais voir grâce à la méthode regexp si le 2ème caractère de chaque élément contient une majuscule
      count += 1 # si cette hypothèse est vérifiée, je rajoute 1 au compteur à chaque fois
    end
  end
  return count # je renvoie le comptage final de la méthode
...
def compter_contain_maj(tabs)
  count=0 # mise en place d'un compteur
  tabs.each do |tab| # chaque élément du tableau tabs est passé en revue et devient tab
    if tab=~/[A-Z]/ # je vais voir grâce à la méthode regexp si le 2ème caractère de chaque élément contient une majuscule
      count += 1 # si cette hypothèse est vérifiée, je rajoute 1 au compteur à chaque fois
    end
  end
  return count # je renvoie le comptage final de la méthode
...
puts tabs.count # compte le nombre d'éléments dans le tableau

# Combien d'handle contiennent un numéro ?
puts compter_les_nb(tabs) # voir définition

# Combien d'handle contiennent les 4 lettres du prénom "Aude" à la suite (sans faire attention à la casse) ?
puts compter_aude(tabs) # voir définition

# Combien commencent par une majuscule ? 215
puts compter_start_maj(tabs) # voir définition

# Combien contiennent une majuscule ?
puts compter_contain_maj(tabs) # voir définition

# Combien de _ dans tous les pseudos confondus ?
puts compter_underscore(tabs) # voir définition
